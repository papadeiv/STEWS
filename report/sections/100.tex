...
\begin{document}
\newpage
\section{Detecting temporal B- and N-tipping in stochastic dynamical systems}\label{sec2}
     In the first $3$ months of the PhD I reviewed the seminal work \cite{Kuehn11} which proposes the fast-slow framework to derive potential EWSs in temporal SDSs. What follows is a series of observations which I obtained when reviewing and attempting to replicate key results from said paper. 
     This activity has been carried out partly in order to meet the first part of the conditions set by \textit{Milestone 2}.
\subsection{The fast-slow framework}\label{subsec2.1}
Usually in dynamical systems, important qualitative properties are derived by considering fixed values of the parameter or with a parameter that evolves with the same temporal scale of the state variable. Whenever the state variable's and the parameter's dynamics evolve according to different timescale we identify a multiscale evolution that is studied within the framework of fast-slow dynamical systems. 
...
\begin{interpretation*}{}
     From now one we will adopt the following notation concerning time-derivatives in fast-slow system. Considering an arfbitrary time-dependent variable $x$ we denote by
     \begin{equation*}
...
\end{interpretation*}
To simplify the dynamics of $(\ref{eq1.1})-(\ref{eq1.2})$ one usually considers the singular limit for the fast-slow system as $\epsilon\to0$. The equation in fast timescale $(\ref{eq1.1})$ reduces to a traditional dynamical system with fixed parameter
\begin{equation}\label{eq1.3}
   ...
   \end{cases}
\end{equation}
The fast subsystem $(\ref{eq1.3})$, usually referred as the layer equation, realises the fast flow of the system and is completely decoupled from the slow subsystem which is instead derived by taking the singular limit of Equation $(\ref{eq1.2})$ giving
...
Equation $(\ref{eq1.4})$ realises the slow flow which is constrained by those points in phase space that satisfy the condition $f(x,y)=0$.
...
\end{definition}
It is easy to see that the fast flow, intended as the solution of the layer equation $(\ref{eq1.3})$, is realised outside $C$ whereas the slow flow is constrained to be realised onto such manifold.
The whole dynamics can thus be approximated by considering $(\ref{eq1.3})$ and $(\ref{eq1.4})$ separately and the resulting fast and slow trajectories then chained together to reconstruct the fast-slow state flow in $\mathbb{R}^{n+m}$. The following example illustrates this approach. 
\begin{example}[label=ex1.1]{}{}
     ...
     \begin{equation*}
        ...
        \end{cases}
     \end{equation*}
     with the slow parameter $y$ varying linearly with the fast timescale $t$. We also immediately note that the fast subsystem (singular limit of the above) gives us exactly the fold bifurcation normal form for fixed values of $y=y_0$
     \begin{equation*}
          \newprime{x}(t) = -y_0 - x^{2}\,.
     \end{equation*}
    To derive the critical manifold we simply compute the fixed points for the above fast dynamics which will give us indeed the fold bifurcation
    \begin{equation*}\label{eq}
            C=\{(x,y)\in \mathbb{R}^{2}\;:\; y=-x^{2}\}\,.
    \end{equation*}
    With a trivial analysis of the Jacobian $D_{x}f$ we can also easily determine the attracting $C^{(a)}=\{C\,\cap\,\{x>0\}\}$ and repelling $C^{(r)}=\{C\,\cap\,\{x<0\}\}$ submanifolds which will aid us in determining the directions of the fast dynamics from any given initial condition as illustrated in Figure \ref{fig1.1}. 
    \newline
    By choosing any point on $C^{(a)}$ as an initial condition we then can also identify the slow dynamics by tracking the evolution of $y$ onto the submanifold.
    ...
    \newline
    Finally we can follow the slow dynamics until the very end of the critical manifold and by repeating the similar concatenation of final state with the initial condition of a new fast trajectory we are able of build a complete rappresentation of the fast-slow dynamics in the singular limit.
    \newline
    Further below in Figure \ref{fig1.3} we reconstruct such analytical approximation and compare it the numerical trajectory obtained for the non-singular case.
\end{example}
\begin{example_continued}{}{}
   ...
   \begin{figure}[H]
        ...
        \caption{Trajectory of the layer equation (blue dots) starting at initial condition $(x=-0.5,y=-1)$ (purple dot) and ending on the attracting submanifold (yellow dot); such final state $(x=1,y=-1)$ is then chosen as the initial condition for the evolution of the reduced equation whose trajectory (red dots) stay on the critical manifold for until reaching the final state (green dot).}
        \label{fig1.2}
    \end{figure}
 ...
 \begin{example_continued}{}{}
   \begin{figure}[H]
       ...
       \caption{Comparison between a concatenation of fast (blue dots) and slow (red dots) subtrajectories in the singular limit with a fast-slow trajectory in the non-singular regime (yellow solid line) computed with a timescale value of $\epsilon=1\cdot10^{-2}$. Note how the subtrajectories in the singular constitute an ideal approximation of the numerical fast-slow trajectory with the latter featuring a delay in the critical transition (which for the fold bifurcation coincides with the origin) which is predicted and can be quantified by Theorem \ref{thm1.2} and \ref{thm1.2}.}
       \label{fig1.3}
   \end{figure}
 \end{example_continued}
...
\end{theorem}
Those results therefore allow us to fully characherize the critical manifold in terms of both the fast and slow variables
...
\end{equation}
to which the critical manifold $C$ perturbs to in the singular limit $\epsilon\to0$.
...
By remarking that any trajectory onto the critical manifold is, by definition, a slow flow solving the reduced equation, then we can easily deduce that a transition between a slow to fast (repelling) dynamics is necessarily a critical transition for the dynamical system. Therefore any concatenation of a slow flow final state with a fast flow initial condition uniquely identifies a critical transition.
\begin{definition}[\hl{Critical transition}]\label{def1.3}
        Let $\gamma(t)$ be a homeomorphic image of the real subset $(a,b)$ s.t. for each partitioning ${a=t_{0}<t_1,\dots,t_{N-1}<t_N=b}$ the image $\gamma(t_{j-1},t_j)$ is an orientation preserving trajectory of either the layer \eqref{eq1.3} or the reduced equation \eqref{eq1.4} and let $p\in C$ be point where the critical manifold loses its normal hyperbolicity, then $p$ is a critical transition if there exist a candidate trajectory $\gamma$ s.t. $p = \gamma(t_j)$ is a transition between fast and slow regimes and $\gamma(t_{j-1},t_j)$ is normally hyperbolic and attracting.
\end{definition}
\subsection{Embedding stochasticity}\label{subsec2.2}
Motivated by many physical applications, in which the underlying dynamical systems appear to be affected by noise, we will now consider stochastic differential equations (SDEs) as our starting point in deriving EWSs for temporal critical transitions.
...
\end{definition}
The system in \eqref{eq1.7} is in Itô differential form; if both the fast $\sigma_f(x,y)=\sigma_f\in\mathbb{R}$ and slow $\sigma_g(x,y)=\sigma_g\in \mathbb{R}$ stochastic dynamics are constants then we are dealing with an Itp's SDE with additive noise, otherwise it will be characterised by multiplicative noise.
...
For \eqref{eq1.8} we would like to use the results derive in the previous subsection for deterministic fast-slow systems and in particular our aim is to characterise the whole evolution in terms of concatenation of separated fast and slow trajectories.
To that end let us use Definition \ref{def1.3} and analyse the stochastic fast-slow systems in order to identify the regions of $y$ close to a critical transition from those that are far away. 
...
\begin{equation}\label{eq1.9}
   ...
   \end{cases}
\end{equation}
with $A_{\epsilon}(y)=(D_x f)(h_{\epsilon}(y),y) - \epsilon (D_y h_{\epsilon})(y) (D_x g)(h_{\epsilon}(y),y)$. Applying Theorem \ref{thm1.2} again for \eqref{eq1.9} we can construct a new slow manifold for the sample paths of the stochastic fast-slow system
...
With these results we are now capable to fully distinguish states of the sample paths of stochastic fast-slow systems that are on normally hyperbolic and attracting submanifolds as stable regimes that are far away from the critical transitions. It will become fundamental later, when deriving stochastic indicators for such transitions, how to choose an appropriate criterion that will count a sample path as escaped fron the slow manifold $C_{\epsilon}$ and therefore linking together such indicators with the OUP normal form derived in this section. 
\begin{example}[label=ex1.2]{}{}
        ...
        \end{equation*}
        we can easily check that the existance of $h:y\mapsto x$ is guranteed by the implicit function Theorem \ref{thm1.1} given that $C$ is normally hyperbolic at any point.
        It follows similarly easy that the only map $x=h(y)=0,\:\forall y\in \mathbb{R}$ is the null function itself which results in
        \begin{equation*}
        ...
        \end{equation}
        for which $M_{1}, M_{2}$ are both positive constants that need to be quantified in order to compute $\rho$ which will give us the boundaries $\partial N(\rho;C_{\epsilon})=\sqrt{\rho^{2}}$.
        ...
        \end{equation}
        We can the plug in the values for $\alpha,\,\sigma$ and $\epsilon$ back in \eqref{eq1.14} to get $M_{1}=225$.
...
\end{example}
\begin{example_continued}{}{}
        We are now enabled to compute the closure of the neighborhood $N(\rho;C_{\epsilon})$ of the critical manifold $C_{\epsilon}$ within which our sample paths realizations $x(\tau)$ for the OUP (which we recall are normally distributed) will stay bounded with high probability.
     \begin{figure}[H]
         \centering 
...
We are now concerned with concluding the previously established criterion by deriving consistent and robust EWS for systems that are instead close or approaching the tipping point. Theorem \ref{thm1.3} already gives us a first hint in the increase in variance as the neighborhood $N(\rho;C_{\epsilon})$ grows in size. 
To deal with this problem in more general terms let us abandon the sample path point of view and consider instead possibile qualitative changes in the Fokker-planck distribution associated to an ensemble of stochastic processes in the form \eqref{eq1.7}.
\begin{definition}[\hl{Phenomenological bifurcation}]\label{def1.5}
     Let \eqref{eq1.7} be a stochastic fast-slow system and $p_{s}(x,y)$ be a probability density for the stochastic process defined by \eqref{eq1.7} satysfing the stationary Fokker-Plank equation \eqref{eq0.1} then $y=y_{p}$ is a phenomenological bifurcation (P-tipping) if $p_{s}(x,y<y_{p})$ is qualitatively different from $p_{s}(x,y>y_{p})$. 
\end{definition}
\begin{example}[label=ex1.3]{}{}
     ...
     \begin{equation*}
        ...
        \end{cases}
     \end{equation*}
     in the singular limit, reducing the above to the (stochastic) layer equation
     \begin{equation*}
         dx =  f(x,y)\,dt + \sigma_{f}(x,y)\,dW\,,
     \end{equation*}
     ...
     What we will do instead is verifying whether a qualitative change in the stationary Fokker-Planck distribution of this process is in fact an EWS for the transcritical bifurcation point $y=0$. From \eqref{eq0.1} we write the stationary Fokker-Planck equation associated to \eqref{eq1.15}
     ...
     \end{equation}
     We can easily conclude that \eqref{eq1.17} is unimodal in the parameter range $y\in(0, \frac{\sigma^{2}}{2})$ and it undergoes a clear qualitative change for parameter values $y>\frac{\sigma^{2}}{2}$. The parameter value $y_{p}=\frac{\sigma^{2}}{2}$ is therefore a P-tipping for our transcritical normal form with multiplicative noise. 
...
\begin{example_continued}{}{}
     ...
     \end{figure}
\end{example_continued}
We are now equipped with a discriminating criterion to identify both sample paths away from critical transitions and those approaching it; in both cases we observed that the variance of the ensemble constitutes a robust EWS to predict the tipping point. 
...
\end{equation}
We immediately realise that equation \eqref{eq1.19} is defined over the entirety of the real line, meaning that in order to restrict it to a bounded and limited subinterval $[a,b]\subset \mathbb{R}$ we must restrict the entirety of the probability current to never leave such domain, resulting in the specification of reflecting boundary conditions. 
The reason for this immediately follow by the constraint of the stationary Fokker-Planck density to remain normalised in any subdomain $[a,b]$ we restrict ourselves.
...
\end{equation}
The above stationary Fokker-Planck distribution gives us a convenient method to detect potential P-tipping by tracking the parameter evolution of the distribution; this result has practical value since we don't necessarly need to know the deterministic dynamic $f(x,y)$ to observe a qualitative change in \eqref{eq1.20} as long as our ensemble simulations are reliable.
Nevertheless, the occurance of a P-tipping is not always guaranteed and visual monitoring of the distribution histogram is less than ideal for real-life EWS; as such it would be optimal if quantitative measures can be derived from \eqref{eq1.20} to provide us with a definitive EWS for temporal stochastic fast-slow systems.
As already mentioned above, the variance has provided insofar a reliable and consistent signal to anticipate and predict the critical transition. From \eqref{eq1.20} we can specify any normal form $f(x,y)$ and check wether the associated variance is indeed a good EWS for the associated deterministic bifurcation.
...
As a matter of fact for parameter values $y<<0$ away from the deterministic B-tipping, the effect of reflecting boundaries on the stationary distribution are negligible since, as we previously established, the stochastic process is topologically equivalent to the OUP and it evolves bounded within the neighborhood of the slow critical manifold $C_{\epsilon}$.
As we approach the critical transition however $y\approx0$ the density $p_{s}(x; y)$ becomes more confined around the stable equilibria leading to N-tipping to occur with higher and higher probability. This can be easily verified by checking how decreasing the noise level $\sigma$ brings the local maxima of the variance closer and closer to the actual N-tipping at $y=0$.
The second most evident observation concerns the discrepancies between the analytical and numerical variance at the local maxima. In all three cases in fact we observe that the two curves pretty much agree away from the deterministic critical transition however they largely differ as the parameter value gets closer to the local maxima for the analytical variance.
We can delegate such discrepancy to the choice that we made earlier in counting ensemble sample paths as escaped the moment the cross the unstable equilibria. This corresponds to absorbing boundary conditions which quite substantially differ from the reflecting boundary conditions used in our stationary Fokker-Planck density modelling approach especially when boundary and noise effects become dominant (i.e. close to the deterministic critical transition as the basin of attraction shrinks further and further in size).
