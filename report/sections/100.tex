...
\begin{figure}[H]
    ...
    \label{fig1.9}
\end{figure}
\subsection{Identifying the tipping radius of systems with steady parameter drift}\label{subsec2.4}
The last cause for open systems to tip away from a stable equilibria are the so called rate-induced critical transitions (i.e. R-tipping). These occur for systems that may not even feature a B- or N-tipping but rather the rate of change of the parameter (usually referred to as drift) may reach a critical value that causes the dynamic state to tip or slip away from a continuosly change system of attractors. 
\begin{definition}[]\label{def1.6}
        Let $x\in \mathbb{R}^{n}$ be a state variable, $y\in \mathbb{R}$ a time-changing parameter, $R>0$ a constant and $M$ a fixed, stable and invertible linear operator then the linearised system
        \begin{equation}\label{eq1.22}
             \newprime{x}(t)=M(x-x^{*}(y)),
        \end{equation}
        is said to be (adiabatically) tracking the stable equilibria $x^{*}(y)$ if $\forall t\;|x(t)-x^{*}(y(t))|<R$ and viceversa it tips beyond the equilibria if $\exists t_{0}\;\text{s.t.}\;|x(t_0)-x^{*}(y(t_0))|=R$. 
\end{definition}
We can deduce from Definition \ref{def1.6} that the stricly positive quantity $R$ is the tipping radius of our open system \eqref{eq1.22}. To study the dependence of the tipping event from the rate of change of the time-varying parameter we introduce the drift of the stable equilibria
\begin{equation}\label{eq1.23}
     \frac{d}{dt}(x^{*}(y(t))) = \frac{dx^{*}}{dy}\frac{dy}{dt} = r(t)\,.
\end{equation}
With these quantities defined we are ready to define a criterion for R-tipping for systems with steady drift.
\begin{theorem}[label=thm1.4]{}{}
     Let \eqref{eq1.22} be a system with tipping radius $R>0$ and $r(t)=r>0$ is constant (i.e. steady drift of the parameter), then 
     \begin{equation*}
          ||M||^{-1}|r|>R
     \end{equation*}
...
\end{theorem}
\begin{proof}
     Given an initial condition $x(0)=x_{0}$ the general solution of \eqref{eq1.22} is
     \begin{equation*}
         x(t) = e^{Mt}x_{0}+\int_{s=0}^{t}e^{M(t-s)}M\,x^{*}(y(s))\,ds = - \int_{u=0}^{\infty}e^{Mu}M\,x^{*}(y(t-u))\,du\,,
     \end{equation*}
     ...
     \begin{equation*}
             x(t) = -[e^{Mu}x^{*}(y(t-u))]_{0}^{\infty} - \int_{0}^{\infty}e^{Mu}\frac{d}{dt}(x^{*}(y(t-u)))\,du\:\Rightarrow\: x(t) - x^{*}(y(t)) = -\int_{0}^{\infty}e^{Mu}r(y(t-u))\,du\,,
     \end{equation*}
     ...
     \begin{equation*}
         x(t) - x^{*}(y(t)) = M^{-1}r(y(t-u))-\int_{0}^{\infty}e^{Mu}M^{-1}\,\newprime{r}(y(t-u))\,du\,.
     \end{equation*}
     Assuming now steady drift the above simplifies to
     \begin{equation*}
         x(t) - x^{*}(y(t)) = M^{-1}r\;\Rightarrow\;|x(t) - x^{*}(y(t))| = |M^{-1}r|\,.
     \end{equation*}
     ...
     \end{equation*}
     We now recall from Definition \ref{def1.6} the condition for R-tipping which gives us the sufficient condition $||M||^{-1}|r|>R$ for the solution to slip off the stable equilibria thereby causing the rate-induced critical transition. 
\end{proof}
\begin{example}[label=ex1.4]{}{}
     We consider a $2-$dimensional fast-slow dynamical system (in the slow timescale) modelling a compost-bomb instability parametrized in $y(t)$ with steady drift
     \begin{equation}\label{eq1.24}
        \begin{cases}
            \epsilon\dot{x}_{1} = x_{2} + y + x_{1}(x_{1}-1)\,, \\
            \dot{x}_{2} = - x_{1} - x_{1}^{2} - x_{1}^{3} - x_{1}^{4} - x_{1}^{5}\,. 
        \end{cases}
     \end{equation}
    The system has one (globally) asymptotically stable equilibria in $(0,-y)$ hence there are no B-tipping; furthermore, since \eqref{eq1.24} is deterministic there are no N-tipping either. As previously done, we take the singular limit of \eqref{eq1.24} and from the reduced equation we are able to construct the critical manifold approximating the slow variable's $x_{2}(t)$ dynamics 
    \begin{equation}\label{eq1.25}
            C=\{(x,y)\in \mathbb{R}^{2}\;:\;x_{2}=-y-x_{1}(x_{1}-1)\}\,.
    \end{equation}
    with attracting $C^{(a)}=\{C\cap\{x_{1}<\frac{1}{2}\}\}$ and repelling $C^{(r)}=\{C\cap\{x_{1}>\frac{1}{2}\}\}$ submanifolds separated by a saddle (fold) line which is tangent to $C$ along $x_{1}=\frac{1}{2}$. Note that the distance along $x_{1}$ between the stable equilibria and the fold of the critical manifold is exactly $\frac{1}{2}=R$ which will thus provide us with the tipping radius for our system.
We now differentiate the critical manifold w.r.t. the slow timescale to obtain
\begin{align}
        0 =&\;\dot{x_{2}} + \dot{y} + \frac{d}{d\tau}(x_{1}^{2}-x_{1}) = f_{2}(x_{1}, x_{2}) + r + \dot{x}_{1}(2x_{1}-1)\;\Rightarrow \nonumber \\
        \Rightarrow&\;\dot{x}_{1}=-\frac{f_{2}(x_{1}, x_{2})+r}{2x_{1}-1} \,, \label{eq1.26} 
\end{align}
where we have the notation $f_{2}(x_{1}, x_{2}):=\dot{x}_{2}=- x_{1} - x_{1}^{2} - x_{1}^{3} - x_{1}^{4} - x_{1}^{5} = -\sum_{n=1}^{5}x_{1}^{\,n}$ for the slow dynamics as well as the definition for the drif $\dot{y} =: r$.
We immediately realise that \eqref{eq1.26} is singular along the fold of the critical manifold therefore, by recalling that $\tau = \epsilon t\;\Rightarrow\;\frac{dt}{d\tau}=\epsilon^{-1}$, we shift the temporal framework onto the fast timescale $t$ obtaining
\begin{equation}\label{eq1.27}
   \begin{cases}
      \newprime{x}_{1} = r - \sum_{n=1}^{5}x_{1}^{n}  \,, \\
      \newprime{y} = -r(2x_{1}-1) \,, 
   \end{cases}
\end{equation}
The equilibria for \eqref{eq1.27} will finally give us the (parametrised) invariant set of $x_{1}$ points to which trajectories starting within the attracting submanifolds will converge to
\begin{equation*}
        M(r) = \{(x_{1}, x_{2})\in \mathbb{R}^{2}\;:\;\sum_{n=1}^{5}x_{1}^{n}=r\}
\end{equation*}
...
\begin{example_continued}{}{}
    ...
    \begin{subfigure}[b]{0.485\textwidth}
        \centering 
        \includegraphics[keepaspectratio, width = \linewidth]{../figures/CompBomb1.png}
        \label{fig1.10.a}
        \caption{}
    \end{subfigure}
    \hfill 
    \begin{subfigure}[b]{0.485\textwidth}
        \centering 
        \includegraphics[keepaspectratio, width = \linewidth]{../figures/CompBomb2.png}
        \label{fig1.10.b}
        \caption{}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.485\textwidth}
        \centering 
        \includegraphics[keepaspectratio, width = \linewidth]{../figures/CompBomb3.png}
        \label{fig1.10.c}
        \caption{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.485\textwidth}
        \centering 
        \includegraphics[keepaspectratio, width = \linewidth]{../figures/CompBomb4.png}
        \label{fig1.10.d}
        \caption{}
    \end{subfigure}
    \caption{System trajectories (green and yellow curves) for the fast-slow system in \eqref{eq1.24} sampled from two initial conditions (blue dots) within the attracting submanifold $C^{(a)}$ and with timescale separation value $\epsilon = 2\cdot10^{-2}$ and parameter $y$ varied linearly in $[0,2]$ with different, steady, rate of change.
            Notice how as we keep increasing the drift of the parameter while keeping it below the critical value (figures (a) and (b)) the invariant set (solid red line) moves closer to the fold of the critical manifold (here depicted as the solid black parabola only for the initial parameter value of $y=0$) but never crosses it, allowing the trajectories to eventually settle on the invariant line meaning that they will remain close (within tipping radius distance) to the stable equilibria (dashed black line). 
    For a drift corresponding to the critical value $r_{c}$ (figure (c)) we notice large oscillations while the trajectories try to settle along the invariant set which has now moved over and overlapping the fold.
Finally as we keep increasing the drift over the critical value (figure (d)) we clearly see that now the invariant line as crossed over the fold and thus the trajectories are now unable to track the stable equilibria thereby slipping off the critical manifold.}
