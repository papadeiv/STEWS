...
Pb = 0::Float64 # breathing pressure
...
function iip_det!(f, x, neigh, t)
        ...
        Pb = (Pb0*N)/(sum)
        for j=1:N
                f[j] = (1+ exp(-Pb + x[end]*(k/x[j]) + Pb*A*(x[j]^4 + x[neigh[1,j]]^4 + x[neigh[2,j]]^4 + x[neigh[3,j]]^4 + x[neigh[4,j]]^4)*(1::Float64 - x[j] + 1.5::Float64*(1-x[j])^2) + Pi))^(-1) - x[j]
        end
        f[end] = 1::Float64
        return nothing
end
