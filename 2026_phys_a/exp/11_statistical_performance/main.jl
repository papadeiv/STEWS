...
function main()
        #----------------------------------------#
        #   Top panel: simulations' parameters   # 
        #----------------------------------------#
        ...
        for idx_μ in eachindex(μ_set), (stepsize, _) in grid
                mkpath("../../res/data/sim_par/$idx_μ/$stepsize")
        end
        ...
        Threads.@threads :dynamic for idx_sim in eachindex(grid) 
                ...
                generate_samples_sim(stepsize, steps)
                next!(progressbar)
        end

        #---------------------------------------#
        #   Bottom panel: systems' parameters   # 
        #---------------------------------------#

        # Materialise the simulation parameter grid for parallelization
        grid = vec(collect(Iterators.product(σ, ε)))

        # Pre-create the output directories to avoid conflicts among threads
        for idx_μ in eachindex(μ_set), (noise, _) in grid
                mkpath("../../res/data/sys_par/$idx_μ/$noise")
        end
        ...
        Threads.@threads :dynamic for idx_sim in eachindex(grid) 
                # Specify the simulation setup
                noise, speed = grid[idx_sim]

                # Solve the ensemble problems and export the results
                generate_samples_sys(noise, speed)
                next!(progressbar)
        end
